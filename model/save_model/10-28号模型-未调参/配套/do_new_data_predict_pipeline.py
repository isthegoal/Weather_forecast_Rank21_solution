...
print('-------   每天需要修改的参数   【开始预测的时间点  天数   文件地址】--------')
the_pred_file='../data/ai_challenger_wf2018_testb1_20180829-20181028.nc'
the_file_day_number=61
the_strat_pred_time='2018/10/29 03:00:00'
...
def nc_to_csv_gc():
    the_testB_data = nc.Dataset('../data/ai_challenger_wf2018_testb1_20180829-20181028.nc')
    ...
    for index, row in the_common_data.iterrows():
        ...
        list_stat_number.extend(
            [mean_t2m_72, mean_t2m_108, min_t2m_72, min_t2m_108, max_t2m_72, max_t2m_108, mean_rh2m_72, mean_rh2m_108,
             ...
             min_w10m_108, max_w10m_72, max_w10m_108, t2m_57, t2m_79, t2m_57, rh2m_79, w10m_57, w10m_79])
...
def make_feature():
    ...
    with open('./make_sbmit/for_submit_do_data/the_time.pkl', 'rb') as f:
    ...
    rh2m_data_fm['the_hour'] = the_hour
    ...
    w10m_data_fm['the_hour'] = the_hour
...
def do_predict_t2m():
    ...
    the_start_time=[]
    the_index_num=data.iloc[:,0].size
    print('行数：',the_index_num)
    for i in range(0,int(int(the_index_num)/33)):
        for j in range(1,34):
            the_start_time.append(j)
    data['start_hour']=the_start_time
    data['start_hour.1']=the_start_time
    data.pop('res_value')
...
def concat_submit():
    t2m=pd.read_csv('./submit/submit_t2m_2018-10-28 .csv',index_col=0)
    rh2m=pd.read_csv('./submit/submit_rh2m_2018-10-28 .csv',index_col=0)
    w10m=pd.read_csv('./submit/submit_w10m_2018-10-28 .csv',index_col=0)
    the_big=pd.merge(t2m,rh2m,on='AnEn_data')
    the_big=pd.merge(the_big,w10m,on='AnEn_data')
    print(the_big.head())
    the_big.to_csv('./submit/submit_all_ceshi.csv')
def do_pipeline():
    '''
        构建自动化提交的过程，包含以下几步：
